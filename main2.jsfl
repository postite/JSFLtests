...
var Main = function() {
	...
	var _g1 = 0, _g = curr_doc.selection.length;
	while(_g1 < _g) {
		var i = _g1++;
		j.set(curr_doc.selection[i].name,{ x : curr_doc.selection[i].x, y : curr_doc.selection[i].y});
	}
	var res = haxe.Json.stringify(j);
	haxe.Log.trace(FLfile.write(this.path + "pop.json",res),{ fileName : "Main.hx", lineNumber : 29, className : "Main", methodName : "new"});
};
Main.__name__ = true;
Main.setRedirection = function() {
	haxe.Log.trace = Main.myTrace;
}
Main.myTrace = function(v,inf) {
	fl.trace(v);
}
Main.main = function() {
	var app = new Main();
